...
pub fn expect_any(val: Option<Value>) -> Result<Value, AiScriptError> {
    Ok(val.ok_or(AiScriptRuntimeError::ExpectAny)?)
}
    ...
    fn try_from(value: V) -> Result<Self, Self::Error> {
        if let V::Bool(value) = value {
            ...
            Err(AiScriptRuntimeError::TypeMismatch {
                ...
                actual: value.display_type().to_string(),
            })?
        }
    }
}
...
static LANG_VERSION_REGEX: LazyLock<Option<Regex>> =
    LazyLock::new(|| Regex::new(r"^\s*///\s*@\s*([a-zA-Z0-9_.-]+)(?:[\r\n][\s\S]*)?$").ok());
            ...
            V::Fn(value) => match value {
                VFn::Fn { params, .. } => write!(
                    ...
                    params
                        .iter()
                        .map(|param| {
                            if let ast::Expression::Identifier(identifier) = &param.dest {
                                &identifier.name
                            } else {
                                "?"
                            }
                ...
                VFn::FnNative(_) | VFn::FnNativeSync(_) => write!(f, "@( ?? ) {{ native code }}"),
            },
            _ => write!(f, "?"),
        }
    }
...
pub fn node_to_value(node: ast::Expression) -> Option<Value> {
    match node {
