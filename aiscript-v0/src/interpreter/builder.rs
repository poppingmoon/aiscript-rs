...
impl InterpreterBuilder {
        ...
        F: Future<Output = String> + Send + 'static,
    {
        self.readline = Some(Value::fn_native(move |args, _| {
            ...
            let in_ = String::try_from(args.next().unwrap_or_default()).map(&in_);
            async {
                let a = in_?.await;
                Ok(Value::str(a))
            }
    ...
    pub fn in_sync(mut self, in_: impl Fn(String) -> String + Sync + Send + 'static) -> Self {
        self.readline = Some(Value::fn_native(move |args, _| {
            let mut args = args.into_iter();
            let result =
                String::try_from(args.next().unwrap_or_default()).map(|q| Value::str(in_(q)));
            async { result }.boxed()
        }));
        self
    }
        ...
        F: Future<Output = ()> + Send + 'static,
    {
        self.print = Some(Value::fn_native(move |args, _| {
            let mut args = args.into_iter();
            let out = expect_any(args.next()).map(&out);
            async {
                out?.await;
                Ok(Value::null())
            }
    ...
    pub fn out_sync(mut self, out: impl Fn(Value) + Sync + Send + 'static) -> Self {
        self.print = Some(Value::fn_native(move |args, _| {
            let mut args = args.into_iter();
            let result = expect_any(args.next()).map(|v| {
                out(v);
                Value::null()
            });
            async { result }.boxed()
        ...
        states.push((
            ...
            self.print.unwrap_or_else(|| {
                Value::fn_native(|args, _| {
                    let mut args = args.into_iter();
                    let result = expect_any(args.next()).map(|_| Value::null());
                    async { result }.boxed()
                })
            }),
        ));
        states.push((
            ...
            self.readline.unwrap_or_else(|| {
                Value::fn_native(|args, _| {
                    let mut args = args.into_iter();
                    let result =
                        String::try_from(args.next().unwrap_or_default()).map(|_| Value::null());
                    async { result }.boxed()
                })
            }),
        ));
